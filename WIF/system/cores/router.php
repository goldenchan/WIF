            ...
            foreach ( $this->routes as $route => $callback ) {
                ...
                if ( preg_match( '#^'.$route.'$#', $this->url_clean, $matches ) ) {
                    ...
                    if(strpos($callback[1],'<') === 0 && isset($params[$k])){
                        ...
                        $callback = (!method_exists($callback[0].'_Controller',$action)? $this->default_route :array($callback[0],$action)); 
                    }
                    
                    $this->callback = $callback;
                    // Return the callback and params, useful for unit testing
                    return array( 'callback' => $callback, 'params' => $params, 'route' => $route );
                }
            }
