 ...
 */
 abstract Class App_Model extends Model{
	...
	var $validate_db = array();
      ...
      * @var array
      */
	var $_validate_rules_support = array('callback_','compare_','nonempty','preg','db_in','db_notin','badword','rangeIn_');//支持的验证方式
     
     /**
      * 默认验证错误提示
      * @var array
      */
     var $_validate_default_errors = array('nonempty'=>'%field% does not exists','preg'=>'%field% format error','db_in'=>'%field% already exists','db_notin'=>'%field% does not exists','callback_'=>'','compare_'=>'','badword'=>'Include Bad words');
			...
			foreach($rules as $key => $rule)
			{
				if(in_array($rule,$this->_validate_rules_support)||substr($rule,0,9)==='callback_'||substr($rule,0,8)==='compare_'||substr($rule,0,8)==='rangeIn_')
                {
                    if(is_string($error))
                    {
                        ...
                        $this->validate_rules[]=array('field'=>$field,'error'=>$error,'rule'=>$rule);
                    }
                    elseif(is_array($error) && count($error) == count($rules))
                    {
                        $this->validate_rules[]=array('field'=>$field,'error'=>$error[$key],'rule'=>$rule);
                    }
                    else
                    {
    ...
    {
 		if(count($this->validate_rules)===0 || !is_array($data) )
			return;
		...
		foreach($this->validate_rules as $validate_rule)
        {
            ...
            if(is_string($field_data))
                $error = str_replace(array('%field%','%value%'),array($validate_rule['field'],$field_data),$validate_rule['error']);
            else
                $error = $validate_rule['error'];
			switch($validate_rule['rule'])
			{
				...
				case 'db_notin'://数据库中必须不存在
                    ...
                    if(is_string($conditions))
                    {
                        ...
                        if(count($matched_data[1])>0){
                            foreach($matched_data[1] as $k => $field){
                                ...
                                $condition_value  = str_replace(array($matched_data[0][$k],'[',']'),array("'".$data[$field]."'",'',''),$conditions); 
                            }
                        }
                    ...
                    else if(is_array($conditions))
                    {
					    ...
					    foreach($conditions as $condition_field => $condition_value)
					    {
						    ...
						    {
                                $data_field  = str_replace(array('[',']'),'',$condition_value);
						    ...
						    elseif(is_array($condition_value))
						    {
							    foreach($condition_value as $k => $v)
							    {
								    ...
								    {
								    	$data_field = str_replace(array('[',']'),'',$v);
									    if(!isset($data[$data_field]))
									    {
										    throw new Exception('Validation error in '.$this->table.':no data info for field '.$data_field);
									    }
				...
				default://回调函数方式
					...
					if(substr($validate_rule['rule'],0,9)==='callback_')
					{
						...
						if(!method_exists($this,$callback_function)||! is_callable(array($this,$callback_function)))
						{
							throw new Exception('Validation error in '.$this->table.':unrecognised rule for field '.$validate_rule['field']);
						}
					}
					...
					call_user_func_array(array($this,$callback_function),array('data'=>$data,'field'=>$validate_rule['field'],'error'=>$error));
					break; 
			}
		}
	}
    ...
    /*public function update($update_info=array(),$condition=array(),$cache_key = null,$cache_group=null){
        $retval = parent::update($update_info,$condition,$cache_key,$cache_group);
        if(!$retval){
            $this->dbServerError(date('Y-m-d H:i:s').__METHOD__.' error :'.'update_info,'.json_encode($update_info).';;'.'condition,'.json_encode($condition));
        }
        return $retval;
    }
    ...
    public function save($insert_info = array(),$cache_group = null){
        try{
            $retval = parent::save($insert_info,$cache_group);
        }
        catch(Exception $e){
           // var_dump($retval);
        }
        if(!$retval){
            $this->dbServerError(date('Y-m-d H:i:s').__METHOD__.'error :'.'insert_info,'.json_encode($insert_info));
        }
        return $retval;
    }

	function delete($condition=array(),$cache_key = 0,$cache_group=null){
        	$retval = parent::delete($condition,$cache_key,$cache_group);
        	if(!$retval){
            		$this->dbServerError(date('Y-m-d H:i:s').__METHOD__.'error :'.'condition'.json_encode($condition));
        	}
        	return $retval;
    	}

    
	function updateAll($update_info=array(),$condition=array(),$cache_group=null){
        	$retval = parent::updateAll($update_info,$condition,$cache_group);
        	if(!$retval){
            		$this->dbServerError(date('Y-m-d H:i:s').__METHOD__.'error :'.'update_info,'.json_encode($update_info).';;'.'condition,'.json_encode($condition));
        	}
        	return $retval;
    	...
    	function dbServerError($string){
        	echo '服务器开小差了！请稍后重试';
        	append_file(LOG.'db_error.txt',$string."\r\n"); 
        	exit;
    	}*/
 }
