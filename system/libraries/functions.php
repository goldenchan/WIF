...
function word_camelcase($underscore_word) {
    if(strpos($underscore_word,"_")>0 ) {
        ...
        return ucfirst($underscore_word);
}
...
function get_mime_type($filename,$ext=''){
    ...
    $mime_types = array('ppt'=>'application/vnd.ms-powerpoint',
    ...
    if(in_array($ext,array_keys($mime_types)) &&( in_array($mime_type,array('application/msword','application/vnd.ms-office')) || (substr($ext,-1)==='x'&&$mime_type===$mime_types[substr($ext,0,-1)]))){
