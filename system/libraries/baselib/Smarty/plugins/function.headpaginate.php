...
function smarty_function_headpaginate($aParam, &$smarty)
{
...
/*	//首页
	if ($nCurrPage == 1)
    {
		$sOut .= sfp_link_head($sGotoText,1,$sStartText,$sUrl,'previous-off',$nPageSize);
    }
    else
    {
    	$sOut .= sfp_link_head($sGotoText,1,$sStartText,$sUrl,'',$nPageSize);
    }*/
	//上一页
    if ($nCurrPage == 1)
    {
		$sOut .= sfp_link_head($sGotoText,1,$sPrevText,'','previous-off',$nPageSize);
    }
    else
    {
		//$sOut .= sfp_link_head($sGotoText,1,$sStartText,$sUrl,'');
		$sOut .= sfp_link_head($sGotoText,$nCurrPage - 1,$sPrevText,$sUrl,'previous',$nPageSize);
    }
    if ($nPageStart > 1)
    {
        $sOut .= $sEtcText;
    ...
    for ($i = $nPageStart; $i <= $nPageEnd; $i++)
    {
        if ($i == $nCurrPage)
        {
            $sOut .= sfp_link_head($sGotoText,$i,$i,$sUrl,'active',$nPageSize);//"<a href='$sUrl1' title='$sGotoText $i'><span class='current'>$i</span></a>";
        }
        else
        {
            $sOut .= sfp_link_head($sGotoText,$i,$i,$sUrl,'',$nPageSize);//"<a href='$sUrl1' title='$sGotoText $i'>$i</a>";
        }
    }
	...
	if ($nCurrPage >= $nPageCnt)
    {
		$sOut .= sfp_link_head($sGotoText,$nCurrPage,$sNextText,$sUrl,'next-off',$nPageSize);
    }
    else
    {

		$sOut .= sfp_link_head($sGotoText,$nCurrPage+1,$sNextText,$sUrl,'next',$nPageSize);
		//$sOut .= sfp_link_head($sGotoText,$nPageCnt,$sEndText,$sUrl,'');
    }
    
/*    //末页
    if ($nCurrPage >= $nPageCnt)
    {
		$sOut .= sfp_link_head($sGotoText,$nPageCnt,$sEndText,$sUrl,'next-off',$nPageSize);
    }
    else
    {
    	$sOut .= sfp_link_head($sGotoText,$nPageCnt,$sEndText,$sUrl,'',$nPageSize);
    }*/
...
function sfp_link_head($goto_text,$tn,$i,$url,$class='',$nPageSize)
{
	...
	{
		return "<span style='color:#666;padding-left: 15px;'>$i</span>\n";
	}
}
/* vim: set expandtab: */