...
class Db_Base {
    ...
    public static function getDb($dsn) {
        if( !isset( self::$db[$dsn] )) {
            try{
                self::$db[$dsn] = new PDO(WI_CONFIG::$dbs[$dsn]['dsn'], WI_CONFIG::$dbs[$dsn]['user'], WI_CONFIG::$dbs[$dsn]['password'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
    ...
    public function query($query, $params=array())
    {
        try{
            ...
            elseif(preg_match("/^(delete|update|drop|create)\s+/i",$query))
